    ...
    class AppDelegate: UIResponder, UIApplicationDelegate {
        ...
        func application(_ application: UIApplication, didFinishLaunchingWithOptions launchOptions: [UIApplicationLaunchOptionsKey: Any]?) -> Bool {print("entered createCDContainer")
            createCDContainer { container in
                print("entered createCDContainer 2")
                self.persistentContainer = container
                container.viewContext.mergePolicy = NSMergeByPropertyObjectTrumpMergePolicy
                let storyboard = self.window?.rootViewController?.storyboard
                print("set storyboard")
                guard let vc = storyboard?.instantiateViewController(withIdentifier: "TabBarController")// as? EventsController
                    else { fatalError("Cannot instantiate root view controller") }
                let ec = vc.childViewControllers[0].childViewControllers[0] as? EventsController

                print("pre set moc")
                ec?.managedObjectContext = container.viewContext
